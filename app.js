...
function newGame() {
  removeClass(document.getElementById("games"), "over");
...
function gameOver() {
  ...
  addClass(document.getElementById("games"), "over");
  // alert("added");
  // wordPerMin=wpm();
  console.log(wpm());
...
document.getElementById("games").addEventListener("keyup", (ev) => {
  const key = ev.key;
  ...
  if (document.querySelector("#games.over")) {
    return;
  }
  if (!window.timer && isLetter) {
    window.timer = setInterval(() => {
      ...
      const secPassed = Math.round(msPassed / 1000);
      const secLeft = Math.round(gameTime / 1000 - secPassed);
      if (secLeft <= -1) {
        gameOver();
        return;
      }
  ...
  if (isLetter) {
    if (currentLetter) {
      ...
      // addClass(incorrectLetter,'hatao');
  ...
  if (isBackSpace) {
    ...
    if (!currentLetter) {
      ...
      const extraLetter = document.querySelectorAll(".letter.extra");
      extraLetter.forEach((el) => {
        el.innerHTML = "";
